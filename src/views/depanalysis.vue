    ...
    <div class="bg">
      ...
      <div class="con_bg">
        ...
        <div class="select_wrap">
            <span class="title">{{analyProInfo.name}}属性选择：</span>
            ...
            <table class="res_wrap">
               <tr class='table-top'><th>名称</th><th>匹配度</th><th>真实性</th></tr>
               <tr v-for="item in depAnalyRes" class="res_item" @click="resShow(item.id)" ><td class="name">{{item.content}}</td><td>{{item.matching}}</td><td>{{item.validity}}</td></tr>
            </table> 
            <div class="more" v-if="hasMore">查看更多</div>
      </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        hasMore(){
          return this.depAnalyRes.length>0
        },
        selectOpts() {
            let _this = this;
